...
def segment_series(scenario, method, threshold, ref_metric):
    ...
    is_vwcd = method.startswith("vwcd")
    ...
    for file_name in [f for f in os.listdir(input_dir) if f.endswith('.csv')]:
        ...
        if is_vwcd:
            ...
            ref_col = f"CP_{ref_metric}" if ref_metric != "full" else None
            if ref_metric != "full" and ref_col not in df.columns:
                continue
            ...
            if ref_metric == "full":
                ...
                if not cp_cols:
                    continue
                df['is_change'] = df[cp_cols].eq(1).any(axis=1)
            else:
                df['is_change'] = df[ref_col] == 1
        ...
        for i, (seg_id, group) in enumerate(groups):
            ...
            if is_vwcd:
                ...
                cp_cols_exist = [col for col in df.columns if col.startswith("CP_")]
                for col in cp_cols_exist:
                    if col in last_row and last_row[col] == 1:
                        metrics_above_threshold += 1
    ...
    if all_segments_data:
        ...
        if is_vwcd:
            ...
            output_path = os.path.join(output_dir, f"{method}_{ref_metric}.csv")
...
if __name__ == "__main__":
    scenario = "NDT"
    threshold = 0.90
    # methods = ["vwcd_w20_fp1","vwcd_w20_fp2", "vwcd_w20_fn2", "vwcd_w24_fp1","vwcd_w24_fp2", "vwcd_w24_fn2"]
    methods = ["vwcd_w24_fn2"]
    ref_metric = "rtt_down"
