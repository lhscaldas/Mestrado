...
def plot_compare(cenario, file, threshold, tail_plot=False, show=False, save=False):
    ...
    ax1.set_title(f'Change Point Detection - {file}')
    ...
    if save:
        if tail_plot:
            dir_path_serie = os.path.join('plots', cenario, 'compare/ts_tail_plot')
        else:
            dir_path_serie = os.path.join('plots', cenario, 'compare/ts_plot')
...
def plot_one(cenario, file, threshold, save=False, tail_plot=False, show=False):
    try:
        plot_compare(cenario, file, threshold, save=save, show=show, tail_plot=tail_plot)
    except Exception as e:
        print(f"Error plotting {file}: {e}")
    

def plot_folder(cenario, threshold, save=True, tail_plot=False):
    serie_folder = f'series/{cenario}'

    for file in os.listdir(serie_folder):
        if file.endswith('.csv'):
            plot_one(cenario, file, threshold, save=save, show=False, tail_plot=tail_plot)

if __name__ == "__main__":
    cenario = "teste"
    # file = "teste01.csv"
    # plot_one(cenario, file, 0.95, save=False, show=True, tail_plot=False)

    plot_folder(cenario, 0.95, save=True, tail_plot=False)
    plot_folder(cenario, 0.95, save=True, tail_plot=True)
